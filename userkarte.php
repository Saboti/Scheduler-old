...
imagestring ($image, 2,0,0,'stgc.fam-hinrichsen.de - '.gmdate('d.m.y H:i', (time() +TIME_OFFSET)).' - Brown Bobby - '.$num.' Spieler', $color_1);
...
foreach($daten as $xpos => $datensatz) {
   foreach ($datensatz as $ypos => $datensatz2) {
      ...
      $color=imagecolorallocate($image,$intensity,$intensity,$intensity);
		...
		$map_data.='\', CAPTION, \'Spieler:\', WIDTH, 300, FGCOLOR, \'#ffffff\', TEXTCOLOR, \'#ffffff\', FGBACKGROUND,\'http://stgc.fam-hinrichsen.de/stgw_gfx/skin1/bg_stars1.gif\', BGCOLOR, \'#687b88\', BORDER, 2, CAPTIONFONT, \'Arial\', CAPTIONSIZE, 2, TEXTFONT, \'Arial\', TEXTSIZE, 2);" onmouseout="return nd();">
		';
	}
}
